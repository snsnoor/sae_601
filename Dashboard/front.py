...
def page_vue_globale():
    ...
    if DB_READY:
        ...
        region_selectionnee = st.selectbox("🌍 Filtrer par région", liste_regions)
            ...
            if DB_READY:
                ...
                if not df_prix.empty:
                    ...
                    fig_map = px.scatter_mapbox(
                        df_prix, lat="latitude", lon="longitude", color="prix_m2_median", size="volume_ventes",
                        hover_name="nom_commune", color_continuous_scale="YlOrRd",
                        range_color=[df_prix['prix_m2_median'].quantile(0.05), df_prix['prix_m2_median'].quantile(0.85)],
                        mapbox_style="carto-positron", zoom=zoom_level, center={"lat": 46.2276, "lon": 2.2137},
                        opacity=0.8, labels={'prix_m2_median': 'Prix médian (€/m²)', 'volume_ventes': 'Nb transactions'}
                    )
...
def charger_et_preparer_donnees(commune, type_local, nb_pieces, surface, budget_min, budget_max, prix_m2_max, dist_gare, dist_ecole):
    ...
    df_display = df[[
        ...
        "nombre_pieces_principales", "prix_m2", "prix_m2_commune", "Évaluation", "dist_gare_km", "dist_ecole_km"
    ]].rename(columns={
        "nom_commune": "Commune", "valeur_fonciere": "Prix (€)",
        "surface_reelle_bati": "Surface (m²)", "nombre_pieces_principales": "Pièces",
        "prix_m2": "€/m² (Bien)", "prix_m2_commune": "€/m² (Ville)",
...
def page_trouver():
    ...
    with col_map:
        ...
        if df_results.empty:
            st.warning("Aucun résultat avec ces critères. Essayez d'augmenter le budget ou la distance.")
        else:
            st.caption(f"{len(df_results)} transaction(s) trouvée(s) — Cliquez sur une ligne pour l'analyser")
        ...
        if not df_results.empty:
            ...
            if lignes_selectionnees:
                ...
                fig_gauge = go.Figure(go.Indicator(
                    ...
                    value = prix_bien_m2,
                    title = {'text': f"<b>{nom_ville}</b> — {statut_affaire}", 'font': {'size': 14, 'family': 'Inter'}},
                    delta = {'reference': prix_ville_m2, 'increasing': {'color': "#D62828"}, 'decreasing': {'color': "#2D6A4F"}, 'font': {'size': 14}},
                    gauge = {
                        'axis': {'range': [None, max(prix_bien_m2, prix_ville_m2) * 1.3], 'tickfont': {'size': 10}},
                        'bar': {'color': "#495057"},
                        'steps' : [
                            {'range': [0, prix_ville_m2 * 0.9], 'color': "#2D6A4F"},      
                            {'range': [prix_ville_m2 * 0.9, prix_ville_m2 * 1.1], 'color': "#FFC107"},  
                            {'range': [prix_ville_m2 * 1.1, max(prix_bien_m2, prix_ville_m2) * 1.3], 'color': "#D62828"} 
                        ],
                        'threshold' : {'line': {'color': "black", 'width': 3}, 'thickness': 0.75, 'value': prix_ville_m2}
                    }
                ))

                fig_gauge.update_layout(
                    margin=dict(t=50, b=20, l=20, r=20), # On aère les marges
                    height=250, # ⬅️ Hauteur augmentée pour que le texte ne se superpose plus !
                    paper_bgcolor="#faf7f2"
                )
                
                st.plotly_chart(fig_gauge, use_container_width=True)
            else:
                st.info("💡 **Cliquez sur une ligne** du tableau ci-dessus pour comparer précisément ce bien au marché de sa commune.")
...
def page_analyses():
    ...
    with col2:
        ...
        fig2 = go.Figure(go.Scatter(
            ...
            marker=dict(color="#6b9e70", size=5, opacity=0.5),
    ...
    fig_peb.update_layout(
        ...
        yaxis=dict(title="€/m²", showgrid=True, gridcolor="#ede8df", range=[0, base_price_an * 1.3]),
        showlegend=False
    )
